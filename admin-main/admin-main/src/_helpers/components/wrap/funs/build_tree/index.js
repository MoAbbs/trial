...
class StateComponent extends Component {
  ...
  resetBuild = (nextProps)=>{
    const {f_fun, app, fun, view, omits=[], 
      ...
      applyFilters(f_fun, undefined, {props: nextProps}).then(fs=>{
        return applyFilters(extra_fun, undefined, {props: nextProps}).then((extra_fs)=>{
          fs = fields || fs;
          extra_fs = extra_fields||extra_fs
          let i=0;
          const columns = sortBy(toArray(omit(mapValues(merge(pick({...fs, ...extra_fs, actions: merge({}, actions, extra_actions)}, view), extra_view), (d, k)=>({
            dataIndex: k,
            key: k,
            _index: d.index || i++,
          ...
          })), omits)).filter(d=>(applyFilters({key: 'list/Every', funs: d.per}))), ['_index'])
          const funs = fun || {key: 'list/ToArray', path: `${app}`, then: {
            key: 'list/build_tree',
          }};
